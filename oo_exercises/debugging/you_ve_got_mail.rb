...
=begin
We have this error message because #send method that is called is not the one from
Mailing module but the default Object#send method. Object#send method tries to invoke
method identified by the first argument (here '860 Blackbird Ln.'). That is why we have
this error message.

This is a case of accidental method overriding.

We need to mix in Mailing module within PostalService class in order to fix the issue
and we should rename Mailing#send method in order to avoid accidental method overriding.
=end
